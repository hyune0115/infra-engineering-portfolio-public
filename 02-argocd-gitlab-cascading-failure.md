...
```
  수백 개 ArgoCD Application
  (reconciliation 주기 과도하게 짧음)
              │
              │  Git 상태체크(reconciliation) 반복 조회
              ▼
  ┌───────────────────────────────┐
  │   중앙 GitLab 서버 요청 폭주     │  ← 존재하지 않는 Repo URL 설정으로
  └───────────────────────────────┘     불필요 조회 추가 가중
              │
              ▼
        HTTP 502 응답
              │
              │  git ls-remote 가 502 수신
              ▼
   인증 없이 재요청 → 서버 재인증 요구
              │
              ▼
   credential-store 에서 저장된 인증정보로 재요청
              │
              │  502 상황을 인증 실패로 처리
              ▼
   ★ 로컬 credential 파일 자동 삭제(erase) ★
              │
              ▼
     이후 모든 Git 인증 지속 실패
              │
